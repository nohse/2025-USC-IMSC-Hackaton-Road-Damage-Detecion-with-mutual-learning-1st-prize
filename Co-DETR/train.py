...
def set_custom_config(cfg):
    # ➊ hack_data 루트
    data_root = '/home/sejong/seonghyeon/road_damage_detection/yolodata/hack_data'

    # ➋ COCO 포맷 어노테이션 사용
    from mmdet.datasets import CocoDataset  # ensure import
    ...
    # ➌ 입력 해상도를 600×600으로 조정
    for pipe in (cfg.train_pipeline, cfg.test_pipeline):
        for step in pipe:
            if step.type == 'Resize':
                step.scale = (600, 600)
    ...
    # evaluator 에도 val JSON 지정
    cfg.val_evaluator.ann_file = f'{data_root}/annotations/val.json'

    # 베스트 체크포인트 자동 저장 설정
    cfg.default_hooks.checkpoint.save_best = 'auto'

    # COCO 사전학습된 Co‑DETR 체크포인트 로드
    cfg.load_from = (
        '/home/sejong/seonghyeon/road_damage_detection/Co-DETR/'
